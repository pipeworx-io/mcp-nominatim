...
`tools/list` at `https://gateway.pipeworx.io/nominatim/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_address \
  -H 'Content-Type: application/json' \
  -d '{"query":"Eiffel Tower, Paris"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_address`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
